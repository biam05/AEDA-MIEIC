...
TEST(test_1, test_a) { //TODO
	...
	ASSERT_EQ(1, pepperoni.getId());
	ASSERT_EQ(2, tuna.getId());
	ASSERT_EQ(3, meat.getId());
}
...
TEST(test_1, test_b) { //TODO
	...
	ASSERT_EQ(true, isSet(v1));
	ASSERT_EQ(false, isSet(v2));
	ASSERT_EQ(true, isSet(v3));
	ASSERT_EQ(true, isSet(v4));*/
}
...
TEST(test_1, test_d) { //TODO
	...
	try {
		ASSERT_EQ(2, marios.optionsWithinIngredientLimits(3, 5).size());
		ASSERT_EQ(0, marios.optionsWithinIngredientLimits(1, 2).size());
		ASSERT_EQ(0, marios.optionsWithinIngredientLimits(10, 15).size());
		ASSERT_EQ(1, marios.optionsWithinIngredientLimits(7, 7).size());
	}
...
TEST(test_1, test_e) { //TODO
	...
	ASSERT_EQ(25, rank[0].getLikes());
	ASSERT_EQ(25, rank[1].getLikes());
	ASSERT_EQ(10, rank[2].getLikes());

	ASSERT_EQ("chicken", rank[0].getName());
	ASSERT_EQ("tuna", rank[3].getName());
	*/
}
...
TEST(test_1, test_f) { //TODO
	...
	ASSERT_EQ(111333222, marios.chefCustomer()->getNIF());
	...
	ASSERT_EQ(222333555, marios2.chefCustomer()->getNIF());
	*/
}
...
TEST(test_1, test_g) { //TODO
	...
	ASSERT_EQ(2, m1.getIngredients().size());

	m1 = marios.removeIngredient(is1, "olive");
	ASSERT_EQ(4, m1.getIngredients().size());

	m1 = marios.removeIngredient(m1.getIngredients(), "mozzarella");
	ASSERT_EQ(3, m1.getIngredients().size());
	...
	try {
		...
		ASSERT_EQ("tuna", e.getIngredient());
	}
	...
	try {
		...
		ASSERT_EQ("pepperoni", e.getIngredient());
	}
	*/
}
