...
void FEUPConsulting::addAvailability(Student* student, string expertizeAvailable, unsigned cost) {
	...
	while(!it.isAtEnd())
    {
	    ...
	    it.retrieve().getCost() == cost){
            it.retrieve().addConsultant(student);
            return;
	    }
	    it.advance();
    }
	Expertize e = Expertize(expertizeAvailable,cost);
...
vector<Student*> FEUPConsulting::getCandidateStudents(Project* project) const {
    ...
    while(!it.isAtEnd())
    {
        ...
        {
            vector<Student*> aux = it.retrieve().getConsultants();
            for(int i = 0; i < aux.size(); i++)
            {
                if(aux[i]->getCurrentProject().empty())
...
bool FEUPConsulting::assignProjectToStudent(Project* project, Student* student) {
    ...
    vector<Student*> availableStudents = getCandidateStudents(project);

    for(auto& s : availableStudents)
    {
        if(s->getName() == student->getName() && s->getEMail() == student->getEMail())
        {
            project->setConsultant(s);
            s->addProject(project->getTitle());
            return true;
        }
    }
    return false;
